    ...
    components.html(
        ...
        (function() {{
            try {{
                ...
                // Fallback: window.parent mungkin blocked di beberapa config browser
                try {{
...
def check_session() -> bool:
    ...
    # 1. Cek session_state internal
    if _session_still_valid():
        return True

    # 2. Cek query_params (hasil bridge dari JS)
    qp            = st.query_params
    ...
    if access_token and refresh_token:
        st.query_params.clear()  # Bersihkan URL langsung setelah dibaca
        if _restore_from_tokens(access_token, refresh_token, expires_at):
            return True

    # 3. Inject JS reader kalau URL masih kosong
    if not access_token:
        _js_read_storage()
